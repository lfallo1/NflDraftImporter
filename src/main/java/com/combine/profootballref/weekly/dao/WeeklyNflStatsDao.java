...
public class WeeklyNflStatsDao {
	...
	"game_identifier, game_link, date, game_number, week, day, year_id, gametime, local_time, league, result, season_type, home_team_identifier, away_team_identifier) " +
	"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
	...
	private static final String INSERT_GAME_STATS = "INSERT INTO nfl.game_team_stats(game_identifier, team_identifier, fourth_down_pct, fourth_down_success, pass_cmp_perc, pass_rating, rush_yds_per_att, temperature, third_down_pct, third_down_success, yds_per_play_defense, yds_per_play_offense, first_down, first_down_pass, first_down_penalty, first_down_rush, fourth_down_att, " +
	" half_1_score_opp, half_1_score_tgl, half_2_score_opp, half_2_score_tgl, pass_att, pass_cmp, pass_int, pass_sacked, pass_sacked_yds, pass_td, pass_yds, penalties, penalties_opp) " +
	"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
	...
	public Integer insertGame(Game game){
		try{
			return this.jdbcTemplate.update(INSERT_GAME, new Object[]{game.getGameIdentifier(), game.getGameLink(), game.getDate(), game.getGameNumber(), game.getWeek(),
					...
					game.getAwayTeam().getTeamIdentifier()});
	...
	public int insertGameStats(WeeklyStatsGame weeklyStatsGame) {
		try{
			return this.jdbcTemplate.update(INSERT_GAME_STATS, new Object[]{weeklyStatsGame.getGameIdentifier(), weeklyStatsGame.getTeamIdentifier(), weeklyStatsGame.getFourth_down_pct(), weeklyStatsGame.getFourth_down_success(),
