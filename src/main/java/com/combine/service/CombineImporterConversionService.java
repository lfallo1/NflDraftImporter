...
@Service
public class CombineImporterConversionService {
    ...
    public Integer collegeNameToId(String value) {
        List<College> colleges = this.combineDao.getColleges();
        ...
        //specific mappings
        if (value.equals("Miami (FL)")) {
            return colleges.stream()
                    .filter(c -> c.getName().equals("Miami"))
                    .map(c -> c.getId())
                    .findFirst()
                    .get();

        } else if (value.equals("Southern Methodist")) {
            return colleges.stream()
    ...
    public Player findPlayerByNflData(String firstname, String lastname, String college, String conference,
        ...
        return this.combineDao.findByAttributes(firstname.toLowerCase(), lastname.toLowerCase(),
                college.toLowerCase(), conference.toLowerCase(), position.toLowerCase());
    }

    /**
     * given a string, return a substring representing the text between two strings
     *
     * @param inputString
     * @param start
     * @param end
     * @return
     */
    public String parseTextParts(String inputString, String start, String end) {
        int startIdx = inputString.indexOf(start) + start.length();
        int endIdx = inputString.substring(startIdx).indexOf(".") > -1 ? startIdx + inputString.substring(startIdx).indexOf(end) : inputString.length();
        return startIdx > -1 ? inputString.substring(startIdx, endIdx) : "";
    }

    /**
     * given a string, return all text after the specified start string
     *
     * @param inputString
     * @param start
     * @param length
     * @return
     */
    public String parseTextParts(String inputString, String start, int length) {
        int startIdx = inputString.indexOf(start) + start.length();
        return startIdx > -1 ? inputString.substring(startIdx, startIdx + length) : "";
    }

    /**
     * @param valueFirst - if value comes as the first arg instead of second
     * @param val
     * @param <S>
     * @param <T>
     * @return
    ...
    public <S, T> Map<S, T> mapOf(boolean valueFirst, Object... val) throws ExceptionInInitializerError {
        if (val.length % 2 == 0) {
            ...
            for (int i = 0; i < val.length; i += 2) {
                if (valueFirst) {
                    map.put((S) val[i + 1], (T) val[i]);
                } else {
                    map.put((S) val[i], (T) val[i + 1]);
                }
            }
            return map;
        }
